        ...
        <div class="accordion file-block-page " id="organizationPagesAccordion">
            @if (isset($data['organization_pages']) && $data['organization_pages']->count() > 0)
                @foreach ($data['organization_pages'] as $key => $item)
                    <div class="clone-file-page">
                        <div class="accordion-item">
                            <h2 class="accordion-header" id="heading{{ $key }}">
                                <button class="accordion-button {{ $key == 0 ? '' : 'collapsed' }}" type="button"
                                        data-bs-toggle="collapse" data-bs-target="#page-collapse{{ $key }}"
                                        aria-expanded="{{ $key == 0 ? 'true' : 'false' }}"
                                        aria-controls="page-collapse{{ $key }}">
                                    Page {{ $key + 1 }}
                                </button>
                            </h2>
                            <div id="page-collapse{{ $key }}"
                                 class="accordion-collapse collapse {{ $key == 0 ? 'show' : '' }}"
                                 aria-labelledby="page-heading{{ $key }}" data-bs-parent="#organizationPagesAccordion">
            ...
            @else
                <div class="clone-file-page">
                    <div class="accordion-item">
                        <h2 class="accordion-header" id="page-heading0">
                            <button class="accordion-button" type="button" data-bs-toggle="collapse"
                                    data-bs-target="#page-collapse0" aria-expanded="true"
                                    aria-controls="page-collapse0">
                                New Page
                            </button>
                        </h2>
                        <div id="page-collapse0" class="accordion-collapse collapse show"
                             aria-labelledby="page-heading0"
